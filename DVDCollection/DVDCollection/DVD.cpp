...
This program is the class for the DVD object. It contains the variables,
movieTitle, movieLength, releaseYear, and an actor object vector(defined in 
Actor.cpp). There are appropriate getters and setters for these variables.
...
DVD::DVD(string mT, double l, int yR, string a, string c)
{
	movieTitle = mT;
	lengthOfMovie = l;
	yearReleased = yR;
	addActorAndCharacter(a, c);
}

void DVD::addActorAndCharacter(string actor, string character)
{
	actors.push_back(Actor(actor,character));
}

void DVD::setLengthOfMovie(double length)
{
	lengthOfMovie = length;
}
...
void DVD::setMovieTitle(string title)
{
	movieTitle = title;
}

void DVD::setActor(int index, string actor, string character)
{
	actors[index].setActorName(actor);
	actors[index].setCharacterName(character);
}

Actor DVD::getActor(int index)
{
	return actors[index];
}

vector<Actor> DVD::getActors()
{
	return actors;
}

int DVD::getLengthOfMovie()
{
